...
from isegm.data.OtherDataset import OtherDataset
from isegm.data.davis import DavisDataset
from isegm.data.sbd import SBDEvaluationDataset
from isegm.model.is_deeplab_model import get_deeplab_model_seresnet
...
def load_is_model(checkpoint, device, backbone='auto', **kwargs):
    ...
    if backbone == 'auto':
        ...
        return load_hrnet_is_model(state_dict, device, backbone, **kwargs)
    elif 'seres' in backbone :
        return load_seresnet_is_model(state_dict, device, backbone, **kwargs)
    else :
        raise NotImplementedError('Unknown backbone')

    # 加载注意力机制的权重
def load_seresnet_is_model(state_dict, device, backbone='auto', deeplab_ch=128, aspp_dropout=0.2,
...
def get_dataset(dataset_name, cfg):
    if dataset_name == 'GrabCut':
        ...
        dataset = OtherDataset(cfg.OTHER_TEST_PATH)
